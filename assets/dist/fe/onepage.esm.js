...
function E(s) {
  return A(s, "data-fp-resv-section");
}
function F(s, t) {
...
class _ {
  ...
  updateSectionState(t, e) {
    this.state.sectionStates[t] = e;
  }
  getSectionState(t) {
    return this.state.sectionStates[t] || "locked";
  }
  markFieldAsTouched(t) {
    this.state.touchedFields[t] = !0;
  }
  isFieldTouched(t) {
    return !!this.state.touchedFields[t];
  }
  setHintOverride(t) {
    this.state.hintOverride = t;
  }
  getHintOverride() {
  ...
  setCtaEnabled(t) {
    this.state.ctaEnabled = t;
  }
  isCtaEnabled() {
    return this.state.ctaEnabled;
  }
  setStarted(t) {
    this.state.started = t;
  }
  isStarted() {
    return this.state.started;
  }
  setFormValidEmitted(t) {
    this.state.formValidEmitted = t;
  }
  isFormValidEmitted() {
...
function I(s) {
  ...
  for (; r < t.length; ) {
    const a = t.length - r;
    let o = e[n % e.length];
    a <= 4 && (o = a), i.push(t.slice(r, r + o)), r += o, n += 1;
  }
  return { masked: i.join(" "), digits: t };
}
function y(s, t) {
  const e = s.value, { masked: i } = I(e), r = s.selectionStart;
  if (s.value = i, r !== null) {
    const n = i.length - e.length, a = Math.max(0, r + n);
    s.setSelectionRange(a, a);
  }
  s.setAttribute("data-phone-local", c(s.value)), s.setAttribute("data-phone-cc", p(t));
}
function g(s, t) {
  const e = c(s.value), i = p(t);
  return {
    e164: k(i, e),
    local: e,
    country: i
...
class q {
  constructor(t, e, i, r, n) {
    this.form = t, this.phoneField = e, this.phonePrefixField = i, this.phoneCountryCode = r, this.copy = n;
  }
  isSectionValid(t) {
    ...
    if ((t.getAttribute("data-step") || "") === "slots") {
      const n = this.form ? this.form.querySelector('[data-fp-resv-field="time"]') : null, a = this.form ? this.form.querySelector('input[name="fp_resv_slot_start"]') : null, o = n && n.value.trim() !== "", l = a && a.value.trim() !== "";
      if (!o || !l)
        return !1;
    }
    let r = !0;
    return Array.prototype.forEach.call(e, function(n) {
      typeof n.checkValidity == "function" && !n.checkValidity() && (r = !1);
    }), r;
  }
  findFirstInvalid(t) {
  ...
  validatePhoneField() {
    if (!this.phoneField)
      return;
    const t = g(this.phoneField, this.phoneCountryCode);
    if (t.local === "") {
      this.phoneField.setCustomValidity(""), this.phoneField.removeAttribute("aria-invalid");
      return;
    }
    return m(t.local) ? (this.phoneField.setCustomValidity(""), this.phoneField.setAttribute("aria-invalid", "false"), !0) : (this.phoneField.setCustomValidity(this.copy.invalidPhone), this.phoneField.setAttribute("aria-invalid", "true"), !1);
  }
  validateEmailField(t) {
    ...
    return t.value.trim() === "" ? (t.setCustomValidity(""), t.removeAttribute("aria-invalid"), !0) : (t.setCustomValidity(""), t.checkValidity() ? (t.setCustomValidity(""), t.setAttribute("aria-invalid", "false"), !0) : (t.setCustomValidity(this.copy.invalidEmail), t.setAttribute("aria-invalid", "true"), !1));
  }
  updateInlineErrors(t, e) {
    if (!this.form)
      return;
    const i = {
      ...
      first_name: e?.messages?.required_first_name || "Inserisci il nome",
      last_name: e?.messages?.required_last_name || "Inserisci il cognome",
      email: this.copy.invalidEmail,
      phone: this.copy.invalidPhone,
      consent: e?.messages?.required_consent || "Accetta la privacy per procedere"
    };
    Object.keys(i).forEach((n) => {
      const a = i[n], o = this.form.querySelector(`[data-fp-resv-error="${n}"]`);
      if (!o)
        return;
      if (n === "consent" && !t[n]) {
      ...
      let l = !1, d = "";
      if (a && typeof a.checkValidity == "function" && !a.checkValidity() && (l = !0, d = r[n] || ""), n === "email" && a && a.value && a.value.trim() !== "" && a.checkValidity() && (l = !1, d = ""), n === "phone" && this.phoneField) {
        const h = g(this.phoneField, this.phoneCountryCode);
        h.local && !m(h.local) && (l = !0, d = this.copy.invalidPhone);
      }
      n === "consent" && a && a.checked && (l = !1, d = ""), l ? (o.textContent = d, o.hidden = !1, a && a.setAttribute && a.setAttribute("aria-invalid", "true")) : (o.textContent = "", o.hidden = !0, a && a.removeAttribute && a.removeAttribute("aria-invalid"));
    });
  }
}
class V {
  constructor(t, e, i, r, n, a) {
    this.sections = t, this.stepOrder = e, this.state = i, this.updateSectionAttributes = r, this.updateProgressIndicators = n, this.updateSubmitState = a;
  }
  getStepOrderIndex(t) {
  ...
  navigateToPrevious(t) {
    ...
    if (!i)
      return;
    const r = i.getAttribute("data-step") || "";
    r && this.activateSectionByKey(r);
  }
  navigateToNext(t, e) {
    const i = t.getAttribute("data-step") || "";
    if (i === "date" || i === "party") {
    ...
    if (i === "slots") {
      const r = this.form ? this.form.querySelector('[data-fp-resv-field="time"]') : null, n = this.form ? this.form.querySelector('input[name="fp_resv_slot_start"]') : null;
      if (!r || r.value.trim() === "" || !n || n.value.trim() === "") {
        const a = this.sections.find((o) => (o.getAttribute("data-step") || "") === "slots");
        if (a) {
          const o = a.querySelector("[data-fp-resv-slots-status]");
          o && (o.textContent = this.copy.slotRequired || "Seleziona un orario per continuare.", o.style.color = "#dc2626", o.setAttribute("data-state", "error"), setTimeout(() => {
            o.textContent = "", o.style.color = "", o.removeAttribute("data-state");
          }, 3e3));
        }
        return;
      }
    }
    if (!e.isSectionValid(t)) {
      const r = e.findFirstInvalid(t);
      r && (typeof r.reportValidity == "function" && r.reportValidity(), typeof r.focus == "function" && r.focus({ preventScroll: !1 }));
      return;
    }
    this.completeSection(t, !0);
  }
  activateSectionByKey(t) {
    ...
    this.sections.forEach((r) => {
      ...
      if (n === t)
        ...
        this.updateSectionAttributes(r, o, { silent: !0 });
      }
  ...
  dispatchSectionUnlocked(t) {
    this.state.unlocked[t] || (this.state.unlocked[t] = !0, this.events.section_unlocked);
  }
  scrollIntoView(t) {
...
function f(s) {
  ...
  if (typeof s == "string")
    return s.trim();
  if (Array.isArray(s))
    return s.map((e) => f(e)).filter((e) => e !== "").join("; ");
  if (typeof s == "object") {
...
const R = typeof window < "u" && typeof window.requestIdleCallback == "function" ? (s) => window.requestIdleCallback(s) : (s) => window.setTimeout(() => s(Date.now()), 1), O = ["date", "party", "slots", "details", "confirm"];
class S {
  constructor(t) {
    this.root = t, this.dataset = C(t), this.config = this.dataset.config || {}, this.strings = this.dataset.strings || {}, this.messages = this.strings.messages || {}, this.events = this.dataset && this.dataset.events || {}, this.integrations = this.config.integrations || this.config.features || {}, this.form = t.querySelector("[data-fp-resv-form]");
  ...
  initializeSections() {
    ...
    this.sections.forEach(function(e, i) {
      ...
      t.state.updateSectionState(r, i === 0 ? "active" : "locked"), i === 0 && t.dispatchSectionUnlocked(r), t.updateSectionAttributes(e, t.state.getSectionState(r), { silent: !0 });
  ...
  initializePhoneField() {
    ...
    this.phoneField && y(this.phoneField, this.getPhoneCountryCode());
  }
  initializeDateField() {
    if (!this.dateField)
      return;
    const t = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    this.dateField.setAttribute("min", t), this.dateField.addEventListener("change", (e) => {
      const i = e.target.value;
      i && i < t ? (e.target.setCustomValidity("Non è possibile prenotare per giorni passati."), e.target.setAttribute("aria-invalid", "true")) : (e.target.setCustomValidity(""), e.target.setAttribute("aria-invalid", "false"));
    });
  }
  initializeAvailability() {
    this.availabilityRoot && this.availabilityRoot.addEventListener("click", (t) => {
      if (this.availabilityController)
        return;
      const e = t.target instanceof HTMLElement ? t.target.closest("button[data-slot]") : null;
      if (!e)
        return;
      t.preventDefault();
      const i = {
        start: e.getAttribute("data-slot") || "",
        label: e.textContent || "",
        status: e.getAttribute("data-slot-status") || ""
      }, r = this.availabilityRoot.querySelectorAll("button[data-slot]");
      Array.prototype.forEach.call(r, (n) => {
        n.setAttribute("aria-pressed", n === e ? "true" : "false");
      }), this.handleSlotSelected(i);
    });
  }
  ...
  handleFormInput(t) {
    ...
    if (!e)
      return;
    this.handleFirstInteraction(), e === this.phoneField ? y(this.phoneField, this.getPhoneCountryCode()) : e === this.phonePrefixField && this.updatePhoneCountryFromPrefix(), this.updateSummary();
    const i = e.getAttribute("data-fp-resv-field") || "", r = i && e.dataset.fpResvLastValue || "", n = i && typeof e.value == "string" ? e.value : "", a = !i || r !== n, o = E(e);
    if (!o) {
  ...
  handleFieldBlur(t) {
    ...
    i && (this.state.markFieldAsTouched(i), i === "phone" && this.phoneField && this.formValidation.validatePhoneField(), i === "email" && e instanceof HTMLInputElement && this.formValidation.validateEmailField(e), this.updateInlineErrors());
  }
  handleKeydown(t) {
    if (t.key !== "Enter")
      return;
  ...
  handleNavClick(t) {
    ...
    const r = e.getAttribute("data-fp-resv-nav");
    console.log("[FP-RESV] Navigation click:", r, "section:", i.getAttribute("data-step")), r === "prev" ? this.formNavigation.navigateToPrevious(i) : r === "next" && this.formNavigation.navigateToNext(i, this.formValidation);
  }
  handleSubmit(t) {
    if (t.preventDefault(), this.state.markFieldAsTouched("consent"), !this.form.checkValidity())
      return this.form.reportValidity(), this.formValidation.focusFirstInvalid(), this.updateInlineErrors(), this.updateSubmitState(), !1;
    ...
    u(e, {
      ...
      party: i.party,
      meal: i.meal
    }), this.preparePhonePayload(), this.state.setSending(!0), this.updateSubmitState(), this.clearError();
    ...
    }).then((l) => (o = Math.round(performance.now() - a), u("ui_latency", { op: "submit", ms: o }), l.ok ? l.json() : l.json().then((d) => {
      const h = d && d.message || this.copy.submitError;
      throw Object.assign(new Error(h), {
        status: l.status,
        payload: d
      });
    }))).then((l) => {
      this.handleSubmitSuccess(l);
    }).catch((l) => {
      o || (o = Math.round(performance.now() - a), u("ui_latency", { op: "submit", ms: o })), this.handleSubmitError(l, o);
    }).finally(() => {
      this.state.setSending(!1), this.updateSubmitState();
    }), !1;
  }
  // Metodi di supporto
  handleFirstInteraction() {
    ...
    u(t, { source: "form" }), this.state.setStarted(!0);
  }
  handleRetrySubmit(t) {
    t.preventDefault(), this.clearError(), this.formValidation.focusFirstInvalid(), this.updateSubmitState();
  }
  ...
  handleSubmitError(t, e) {
    const i = t && typeof t.status == "number" ? t.status : "unknown", r = t && t.message || this.copy.submitError, n = t && typeof t == "object" && t.payload || null, a = L(r, n);
    this.errorAlert && this.errorMessage && (this.errorMessage.textContent = a, this.errorAlert.hidden = !1, requestAnimationFrame(() => {
    ...
    })), this.state.setHintOverride(a), this.updateSubmitState();
    const o = this.events.submit_error || "submit_error";
    u(o, { code: i, latency: e });
  }
  clearError() {
    this.errorAlert && (this.errorAlert.hidden = !0), this.state.setHintOverride("");
  }
  ...
  updateSubmitState() {
    ...
    if (this.state.isSending() ? this.setSubmitButtonState(!1, "sending") : this.setSubmitButtonState(t, null), this.submitHint) {
      const e = this.state.getHintOverride() || (t ? this.state.getState().initialHint : this.copy.ctaDisabled);
  ...
  setSubmitButtonState(t, e) {
    if (!this.submitButton)
      return;
    const i = e === "sending" ? !1 : !!t;
    F(this.submitButton, !i), this.submitLabel && (e === "sending" ? this.submitLabel.textContent = this.copy.ctaSending : i ? this.submitLabel.textContent = this.copy.ctaEnabled : this.submitLabel.textContent = this.copy.ctaDisabled), this.submitSpinner && (this.submitSpinner.hidden = e !== "sending"), this.state.setCtaEnabled(i);
  }
  ...
  getReservationEndpoint() {
    return "/wp-json/fp-resv/v1/reservations";
  }
  isConsentField() {
    return !1;
  }
  handleDelegatedTrackingEvent() {
  }
  ...
  dispatchSectionUnlocked() {
  }
}
typeof window < "u" && (window.FPResv = window.FPResv || {}, window.FPResv.FormApp = S, window.fpResvApp = window.FPResv);
function b() {
  ...
  console.log("[FP-RESV] Found widgets:", s.length), Array.prototype.forEach.call(s, function(t) {
    try {
      new S(t), console.log("[FP-RESV] Widget initialized:", t.id || "unnamed");
    } catch (e) {
