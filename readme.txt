...
* Fixed: dashboard Manager mobile — il campo data sotto i 480px resta largo e centrato tra le frecce invece di comprimersi nella colonna da 44px.

= 1.3.12 - 2026-05-12 =

* Fixed: dashboard Manager più utilizzabile da mobile — header, azioni, filtri, statistiche, switcher vista, modali e vista settimanale adattati a viewport stretti.

= 1.3.11 - 2026-05-08 =

* Fixed: `AdminMenuEnhancer.php` — rimosso codice duplicato dopo la chiusura della classe (stesso rischio di parse error della 1.3.9).

= 1.3.10 - 2026-05-07 =

* Fixed: tracking `booking_confirmed` / `booking_payment_completed` da cambio stato admin — sempre inviati `value` e `currency` (anche importo 0), così FP Tracking non segnala più campi obbligatori mancanti.
...
* Fixed: form «Modifica evento operativo» in modalità avanzata con apertura speciale — layout non più compresso in una colonna stretta; blocco nome servizio / capienza / fasce orarie su riga intera sotto i campi principali; fasce orarie con wrap e input che non vanno in overflow.

= 1.3.7 - 2026-04-26 =

* Changed: aperture speciali e orari speciali senza ricorrenza, con data/ora di fine già passata, vengono disattivati automaticamente all’apertura della lista o della preview del planner (non compaiono più tra gli attivi).

= 1.3.6 - 2026-04-27 =

* Changed: modale «Calendario operativo» più alta — usa quasi tutto il viewport (`calc(100vh - 20px)`), contenuto allineato in stretch; corpo a colonna flex con area planner (`#fp-resv-closures-app`) che occupa lo spazio rimanente e scroll interno.

= 1.3.5 - 2026-04-27 =

* Changed: modale «Calendario operativo» nel Manager — header a gradiente FP, hint in card, statistiche in griglia stile closures, backdrop sfocato, blocco scroll body e focus/restore accessibilità; toolbar planner compatta nel modale (niente titolo duplicato).

= 1.3.4 - 2026-04-26 =

* Changed: il planner «Calendario operativo» si apre in un modale a tutta larghezza dal Manager (niente più schede in pagina). Chiusura con X, backdrop o ESC; link diretto `fp_resv_tab=closures` apre il modale al caricamento.

= 1.3.3 - 2026-04-26 =

* Changed: il planner «Calendario operativo» (chiusure, aperture speciali, riduzioni capienza) è integrato nel Manager con schede «Prenotazioni» e «Calendario operativo». La vecchia pagina `fp-resv-closures-app` reindirizza al Manager con `fp_resv_tab=closures`. Link admin bar, meal plan e testi guida aggiornati.
...
* Fixed: regressione 1.3.0 — utenti con ruolo `fp_manager` venivano redirettati alla pagina "Mio account" di WooCommerce invece che al backend WordPress. Aggiunta la capability `view_admin_dashboard` al ruolo (WooCommerce `prevent_admin_access` richiede una tra `edit_posts` / `manage_woocommerce` / `view_admin_dashboard`). Applicata automaticamente al prossimo caricamento admin.

= 1.3.0 - 2026-04-23 =

* Added: classe `AdminRestrictor` che limita l'UI admin per gli utenti con ruolo `fp_manager`. Nasconde menu top-level non pertinenti (Articoli, temi, page builder, strumenti esterni), tutti i widget della Bacheca e le voci della admin bar estranee a FP Experiences / FP Restaurant Reservations. Gli amministratori non sono toccati.
* Added: filtro `fp_resv_admin_menu_whitelist` per personalizzare la whitelist dei menu top-level visibili al ruolo `fp_manager`.
* Changed: rimossa la capability `edit_posts` dal ruolo `fp_manager`. Non era necessaria (il plugin usa capability custom) e causava la comparsa di menu esterni a FP. Pulizia idempotente applicata automaticamente al primo accesso admin.
...
= 1.1.2 - 2026-04-18 =
* Fixed: Rimosse le righe orizzontali sopra "Servizi Aggiuntivi" e sopra i consensi privacy (border-top sui fieldset introdotto in 1.1.1). Il raggruppamento è ora dato dalla sola legend visibile e da uno spacing verticale più pulito.

= 1.1.1 - 2026-04-18 =
* Fixed: Form frontend — appiattiti i contenitori nidificati che creavano l'effetto "scatola dentro scatola" (steps-container, step, field, fieldset). Ora c'è un solo contenitore principale con sezioni interne sobrie.
* Fixed: Rimossa la barra orizzontale decorativa sopra ogni step (duplicava la barra del container esterno).
* Fixed: Rimossa la barra verticale accanto al titolo di ogni step (il numero "1.", "2.", ecc. è già nel testo, era ridondante).
...
= 1.0.47 - 2026-04-18 =
* Form frontend: progress bar con cerchi 1-2-3-4 raggruppati al centro del form e linea di sfondo che si ferma esattamente al primo e all'ultimo cerchio (fix sovrascrittura da StyleCss.php che portava la linea a tutta larghezza).

= 1.0.46 - 2026-04-18 =
* Form frontend: progress bar compattata al centro (max-width 360px desktop / 320px mobile) con numeri più ravvicinati.

= 1.0.45 - 2026-04-18 =
* Form frontend: progress bar step 1-2-3-4 distribuita a larghezza piena (space-between) con la linea di sfondo agganciata da centro-primo a centro-ultimo step, niente più "vuoto" a destra.

= 1.0.44 - 2026-04-18 =
* Form frontend: bottoni nav nascosti allo step corrente ora forzati con `style.display="none" !important` per battere temi Salient/WPBakery che applicano `button { display: inline-block !important }` (fix "Indietro" visibile allo step 1).

= 1.0.40 - 2026-04-18 =
* Form frontend: CSS token da Impostazioni (Aspetto) iniettato in pagina; layout card/steps/pulsanti/calendario con variabili --fp-resv-*. Flatpickr riceve i token in apertura. Nessuna variazione alle opzioni colore nel backend.
