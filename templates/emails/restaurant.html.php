...
$variant    = isset($variant) ? (string) $variant : 'restaurant';
$labels     = is_array($strings['labels'] ?? null) ? $strings['labels'] : [];

$headlineKey = $variant === 'restaurant' ? 'headline_restaurant' : 'headline_webmaster';
$leadKey     = $variant === 'restaurant' ? 'lead_restaurant' : 'lead_webmaster';

$headline = (string) ($strings[$headlineKey] ?? __('Nuova prenotazione ricevuta', 'fp-restaurant-reservations'));
$lead     = (string) ($strings[$leadKey] ?? __('Il ristorante %s ha ricevuto una nuova prenotazione.', 'fp-restaurant-reservations'));
...
<table role="presentation" cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;font-family:Arial,sans-serif;font-size:14px;color:#222;">
    <tbody>
        ...
        <?php if ($createdAtFormatted !== '') : ?>
            <tr>
                <th align="left" style="padding:4px 8px;font-weight:bold;">
                    <?php echo esc_html($label($labels, 'received_at', __('Ricevuta il', 'fp-restaurant-reservations'))); ?>
                </th>
                <td style="padding:4px 8px;">
...
<p style="margin:16px 0 0;font-family:Arial,sans-serif;font-size:14px;color:#333;">
    <a href="<?php echo esc_url((string) ($reservation['manage_url'] ?? '')); ?>" style="color:#005f73;">
        <?php echo esc_html($label($labels, 'manage', __('Gestisci prenotazione', 'fp-restaurant-reservations'))); ?>
    </a>
</p>
